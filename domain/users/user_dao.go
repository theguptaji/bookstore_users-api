...
import (
	...
	"github.com/theguptaji/bookstore_users-api/utils/mysql_utils"

	"github.com/theguptaji/bookstore_users-api/logger"

	"github.com/theguptaji/bookstore_users-api/datasources/mysql/users_db"
)
...
func (user *User) FindByStatus(status string) ([]User, *rest_errors.RestErr) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&user.Id, &user.FirstName, &user.LastName, &user.Email, &user.DateCreated, &user.Status); err != nil {
			logger.Error("error when scan user into user struct", err)
			return nil, rest_errors.NewInternalServerError("error when trying to get user", errors.New("database error"))
		}
		results = append(results, user)
	}
	if len(results) == 0 {
		return nil, rest_errors.NewNotFoundError(fmt.Sprintf("no users matching the status %s", status))
	}
	return results, nil
}
